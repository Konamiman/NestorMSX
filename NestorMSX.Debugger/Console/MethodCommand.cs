...
namespace Konamiman.NestorMSX.Z80Debugger.Console
{
    public class MethodCommand : Command
    {
        ...
        public MethodCommand(MethodInfo method, object hostingObject)
        {
            this.method = method;
        ...
        public override object Execute(string name, CommandArgument[] arguments)
        {
            ...
            if(namedArguments.Any(a => a.Name == null))
                throw new Exception($"{name}: Only named argument specifications must appear after all fixed arguments have been specified");
            ...
            if(conflictingArgumentNames.Any())
                throw new Exception($"{name}: Named argument '{conflictingArgumentNames.First()}' specifies a parameter for which a positional argument has already been given");
            ...
            if(argumentNamesSpecifiedMoreThanOnce.Any())
                throw new Exception($"{name}: Named argument '{argumentNamesSpecifiedMoreThanOnce.First().Key}' cannot be specified multiple times");
            ...
            if(unespecifiedMandatoryParameterNames.Any())
                throw new Exception($"{name}: There is no argument given that corresponds to parameter '{unespecifiedMandatoryParameterNames.First()}'");

            var unknownArgumentNames =
                namedArguments.Select(a => a.Name).Except(parameterNames).ToArray();
            if(unknownArgumentNames.Any())
                throw new Exception($"{name}: Unknown parameter '{unknownArgumentNames.First()}'");
            ...
            {
                var matchingArgument = arguments.SingleOrDefault(a => a.Name != parameter.Name);
