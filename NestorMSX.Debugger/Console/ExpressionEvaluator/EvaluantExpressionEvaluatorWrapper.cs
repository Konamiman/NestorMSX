    ...
    public class EvaluantExpressionEvaluatorWrapper : IExpressionEvaluator
    {
        ...
        private static string Preprocess(string expression)
        {
            ...
            expression = Regex.Replace(expression,
	            ...
	            @"|"+
	            @"(((0[xX])|(&[Hh])|#)(?<hvalue>[0-9A-Fa-f]+))|((?<hvalue>[0-9][0-9A-Fa-f]*)(?![\(\]])(H|h))"+ //hex numbers
	            @"|"+
	            @"(((&[bB])|%)(?<bvalue>[0-9A-Fa-f]+))|((?<bvalue>[0-9][0-9A-Fa-f]*)(?![\(\]])(B|b))", //bin numbers - [0-F] on purpose to catch FormatExceptions
	            e => e.Groups["hvalue"].Value != "" ? int.Parse(e.Groups["hvalue"].Value, System.Globalization.NumberStyles.HexNumber).ToString() :
		             e.Groups["bvalue"].Value != "" ? Convert.ToInt32(e.Groups["bvalue"].Value, 2).ToString() :
